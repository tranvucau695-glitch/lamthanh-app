  ...
  try {
    ...
    for (const p of products) {
      ...
      for (const imp of imports) {
        for (const item of (imp.items || [])) {
          if ((item.id || item.productId) === pid) totIn += Number(item.q || item.qty || 0);
        }
      }
      ...
      for (const o of orders) {
        ...
        for (const part of parts) {
          ...
          if (m && m[1].trim() === pname) totOut += parseFloat(m[2]) || 0;
        }
      }

      // Tổng trả hàng (hoàn kho)
      let totRet = 0;
      for (const r of returns) {
        for (const item of (r.returnItems || [])) {
          if ((item.productId || '') === pid) totRet += Number(item.qty || 0);
        }
      }
