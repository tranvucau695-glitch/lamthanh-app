...
app.post('/api/imports', async (req, res) => {
  try {
    ...
    for (const item of items) {
      // FIFO: thêm lô mới vào batches, cập nhật tồn kho
      // Không ghi đè cost nữa - cost sẽ được tính theo FIFO khi xuất
      const newBatch = { qty: item.q, cost: item.up, date: date, impId: impId };
      await Product.findByIdAndUpdate(item.id, {
        $inc: { stock: item.q },
        $push: { batches: newBatch }
        // Không $set cost nữa - giữ nguyên để tham khảo, tính FIFO khi bán
      });
