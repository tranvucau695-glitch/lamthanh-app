...
app.post('/api/sync-all', async (req, res) => {
  try {
    // 0. Xóa collections rác
    await Collection.deleteMany({ amount: { $lte: 0 } });

    // 1. Đơn tiền mặt rõ ràng: đảm bảo collected = total
    await Order.updateMany(
      { payType: 'tienmat' },
      [{ $set: { collected: '$total' } }]
    );

    // 2. Đơn nào collected >= total → đã thu đủ, giữ nguyên
    // Đơn nào collected < total VÀ không phải tiền mặt → reset về 0 để tính lại
    await Order.updateMany(
      {
        payType: { $ne: 'tienmat' },
        $expr: { $lt: ['$collected', '$total'] }
      },
      { $set: { collected: 0 } }
    );

    // 3. Phân bổ collections vào đơn còn nợ
    const collections = await Collection.find().sort({ date: 1 });
    for (const col of collections) {
      const custId = col.custId;
      const cust = await Customer.findById(custId).catch(() => null);
      const custIds = [custId];
      if (cust && cust.id) custIds.push(cust.id);

      const colDateISO = col.date ? col.date.split('/').reverse().join('-') : '9999-12-31';

      const getDebtOrders = () => Order.find({
      ...
      // Phân bổ vào đơn cũ hơn hoặc cùng ngày
      const orders = await getDebtOrders();
      for (const o of orders) {
        if (rem <= 0) break;
        const oDateISO = o.date ? o.date.split('/').reverse().join('-') : '0000-01-01';
        if (oDateISO > colDateISO) continue;
      ...
      // Còn dư → phân bổ vào đơn sau
      if (rem > 0) {
        const later = await getDebtOrders();
    ...
    // 4. Tính lại debt cho tất cả khách
    ...
    for (const c of customers) {
      ...
      // Chỉ tính đơn chưa thu đủ (collected < total) và không phải tiền mặt
      const orders = await Order.find({
        customerId: { $in: custIds },
        status: 'delivered',
        payType: { $ne: 'tienmat' },
      ...
      const debt = orders.reduce((s, o) => s + Math.max(0, (o.total || 0) - (o.collected || 0)), 0);
      c.debt = debt;
      await c.save();
      updated++;
    }
