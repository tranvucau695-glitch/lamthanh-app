...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    if (req.body.customerId) {
      ...
      if (cust) {
        cust.total = (cust.total || 0) + (req.body.total || 0);
        cust.debt = (cust.debt || 0) + (req.body.total || 0);
        await cust.save();
      }
    }
...
app.put('/api/returns/:id', async (req, res) => {
  try {
    ...
    // Hoàn lại tồn kho (trừ lại số đã nhập vào kho khi trả)
...
app.delete('/api/cashflow/:id', async (req, res) => {
  try {
    ...
    // Nếu là phiếu thu công nợ → tìm Collection tương ứng, hoàn nợ và xóa collected trên orders
    if (cf.type === 'Thu' && cf.desc && cf.desc.includes('Thu công nợ')) {
      // Tìm collection liên quan (cùng ngày, cùng số tiền)
      const col = await Collection.findOne({ amount: cf.amount, date: cf.date, cust: cf.subject });
      if (col) {
        ...
        if (cust) {
          // Hoàn nợ lại
          cust.debt = (cust.debt || 0) + cf.amount;
          await cust.save();
          // Hoàn collected trên orders (trừ ngược lại)
          let rem = cf.amount;
          const orders = await Order.find({
            customerId: { $in: [custId, cust._id.toString()] }
          }).sort({ createdAt: -1 }); // hoàn từ đơn mới nhất
          for (const o of orders) {
            if (rem <= 0) break;
            const canReturn = Math.min(o.collected || 0, rem);
            if (canReturn > 0) {
              o.collected = (o.collected || 0) - canReturn;
              await o.save();
              rem -= canReturn;
            }
          }
        ...
        // Fallback: tìm khách theo tên và hoàn nợ
        const cust = await Customer.findOne({ name: cf.subject });
        if (cust) {
          cust.debt = (cust.debt || 0) + cf.amount;
          await cust.save();
        }
