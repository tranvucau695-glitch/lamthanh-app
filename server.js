...
app.post('/api/sync-all', async (req, res) => {
  try {
    ...
    // 1. Đơn tiền mặt: collected = total (đã thu đủ, không động vào)
    await Order.updateMany({ payType: 'tienmat' }, [{ $set: { collected: '$total' } }]);

    // 2. Reset collected = 0 cho tất cả đơn KHÔNG phải tiền mặt
    await Order.updateMany(
      { payType: { $ne: 'tienmat' } },
      { $set: { collected: 0 } }
    );

    // 3. Phân bổ collections vào đơn hàng
    const collections = await Collection.find().sort({ date: 1 });
    for (const col of collections) {
      const custId = col.custId; // _id string
      // Tìm customer để lấy cả id lẫn _id
      ...
      // Ưu tiên đơn bán trước hoặc cùng ngày thu
      ...
      // Nếu còn dư → phân bổ vào đơn sau ngày thu
      if (rem > 0) {
        const laterOrders = await getOrders();
    ...
    for (const c of customers) {
      ...
      c.debt = Math.max(0, totalOrdered - totalCollected);
      await c.save();
      updated++;
    }
    res.json({ ok: true, updated });
