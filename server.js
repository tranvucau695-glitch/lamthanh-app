...
// --- SYNC ALL: Tính lại debt từ dữ liệu hiện có ---
app.post('/api/sync-all', async (req, res) => {
  try {
    // Bước 1: Fix đơn tiền mặt collected = total (đơn cũ có thể collected=0)
    const tmOrders = await Order.find({ payType: 'tienmat' });
    for (const o of tmOrders) {
    ...
    // Bước 2: Tính lại debt = sum(total - collected) cho đơn collected < total
