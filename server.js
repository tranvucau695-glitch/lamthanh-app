  ...
  try {
    ...
    for (const c of custs) {
      ...
      result.push({
        ...
        tong_order_total: orders.reduce((s,o)=>s+(o.total||0),0),
        tong_collected: orders.reduce((s,o)=>s+(o.collected||0),0),
        orders: orders.map(o=>({ id:o.id, date:o.date, total:o.total, collected:o.collected, payType:o.payType, customerId:o.customerId }))
      });
    }
    res.json(result);
  } catch(e) { res.status(500).json({ error: e.message }); }
...
// --- SYNC ALL: Tính lại collected từ collections rồi tính debt ---
app.post('/api/sync-all', async (req, res) => {
  try {
    // Bước 1: Fix đơn tiền mặt -> collected = total
    const tmOrds = await Order.find({ payType: 'tienmat' });
    for (const o of tmOrds) {
      if ((o.collected || 0) < (o.total || 0)) {
        o.collected = o.total; await o.save();
      }
    }

    // Bước 2: Reset collected = 0 cho đơn công nợ (không phải tiền mặt)
    // Chỉ reset đơn chưa thu đủ để tính lại từ collections
    ...
    for (const col of allCols) {
      ...
      // Tìm customer để lấy đủ custIds
      let cust = null;
      try { cust = await Customer.findById(custId); } catch(e) {}
      if (!cust) cust = await Customer.findOne({ id: custId });
      if (!cust) continue;
