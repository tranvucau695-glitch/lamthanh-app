...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    if (isTM) {
      // Thu tiền mặt: tạo phiếu thu tiền mặt, không tính công nợ
      await Cashflow.create({
      ...
      // Không cộng vào debt khách hàng
    } else if (req.body.customerId) {
      // Công nợ: cộng debt, tạo phiếu thu công nợ khi thu tiền sau
      let cust = null;
      try { cust = await Customer.findById(req.body.customerId); } catch(e) {}
      if (!cust) cust = await Customer.findOne({ id: req.body.customerId });
      if (cust) {
        cust.debt = (cust.debt || 0) + req.body.total;
        cust.total = (cust.total || 0) + req.body.total;
        await cust.save();
      }
    }
