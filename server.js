...
app.post('/api/sync-all', async (req, res) => {
  try {
    ...
    // 1. Reset collected về 0 cho tất cả orders (trừ đơn tiền mặt đã thu đủ)
    await Order.updateMany({ payType: { $ne: 'tienmat' } }, { $set: { collected: 0 } });
    ...
    for (const col of collections) {
      ...
      const orders = await Order.find({
        ...
        status: 'delivered',
        payType: { $ne: 'tienmat' } // bỏ qua đơn tiền mặt
