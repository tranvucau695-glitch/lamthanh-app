...
app.post('/api/sync-all', async (req, res) => {
  try {
    ...
    // 1. Đơn tiền mặt rõ ràng: collected = total
    await Order.updateMany({ payType: 'tienmat' }, [{ $set: { collected: '$total' } }]);

    // 2. Reset collected = 0 cho TẤT CẢ đơn KHÔNG phải tiền mặt (bao gồm đơn cũ không có payType)
    // Đây là bước quan trọng: phải reset trước khi phân bổ lại từ collections
    await Order.updateMany(
      { payType: { $ne: 'tienmat' } },
      { $set: { collected: 0 } }
    );

    // 3. Phân bổ collections vào đơn hàng theo thứ tự ngày
    ...
    for (const col of collections) {
      ...
      // Lấy tất cả đơn công nợ của khách còn nợ, sắp xếp cũ nhất trước
      const orders = await Order.find({
    ...
    // 4. Tính lại debt cho tất cả khách (chỉ từ đơn không phải tiền mặt)
