...
app.post('/api/sync-all', async (req, res) => {
  try {
    ...
    // 1. Reset collected về 0 chỉ cho đơn công nợ RÕ RÀNG (payType = 'congno')
    // Đơn không có payType (đơn cũ) → giữ nguyên collected
    await Order.updateMany({ payType: 'congno' }, { $set: { collected: 0 } });
    // Đơn tiền mặt: collected = total
    await Order.updateMany({ payType: 'tienmat' }, [{ $set: { collected: '$total' } }]);
    // Đơn cũ không có payType mà collected >= total → coi như tiền mặt (đã thu đủ)
    await Order.updateMany(
      { payType: { $exists: false }, $expr: { $gte: ['$collected', '$total'] } },
      [{ $set: { collected: '$total' } }]
    );

    // 2. Tính lại collected từ collections - phân bổ theo thứ tự ngày thu vs ngày bán
    const collections = await Collection.find().sort({ date: 1 }); // sắp xếp thu cũ nhất trước
    for (const col of collections) {
      const custId = col.custId;
      // Chuyển ngày thu thành string để so sánh
      const colDate = col.date; // dd/mm/yyyy
      const colDateISO = colDate ? colDate.split('/').reverse().join('-') : '9999-12-31';

      // Chỉ phân bổ vào đơn bán TRƯỚC hoặc CÙNG ngày thu, sắp xếp cũ nhất trước
      const orders = await Order.find({
        customerId: { $in: [custId] },
        status: 'delivered',
        payType: { $nin: ['tienmat'] }, // bỏ qua đơn tiền mặt
        $expr: { $gt: ['$total', '$collected'] } // chỉ lấy đơn còn nợ
      }).sort({ date: 1, createdAt: 1 });

      let rem = col.amount;
      for (const o of orders) {
        if (rem <= 0) break;
        // Chỉ áp cho đơn bán trước hoặc cùng ngày thu
        const oDateISO = o.date ? o.date.split('/').reverse().join('-') : '0000-01-01';
        if (oDateISO > colDateISO) continue; // đơn mới hơn ngày thu → bỏ qua
        const due = o.total - (o.collected || 0);
        if (due <= 0) continue;
        const take = Math.min(due, rem);
        o.collected = (o.collected || 0) + take;
      ...
      // Nếu còn dư → phân bổ vào đơn sau ngày thu (thu trước bán)
      if (rem > 0) {
        const laterOrders = await Order.find({
          customerId: { $in: [custId] },
          status: 'delivered',
          payType: { $nin: ['tienmat'] },
          $expr: { $gt: ['$total', '$collected'] }
        }).sort({ date: 1, createdAt: 1 });
